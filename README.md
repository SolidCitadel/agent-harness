...
- `codex/`: Codex의 지침·hook·플랫폼별 skill
...
설치기는 기존 대상을 타임스탬프 백업으로 이동한 뒤 디렉터리는 junction으로 연결한다. 파일 symbolic link 권한이 없는 Windows에서는 같은 볼륨의 hard link를 사용한다. Git 작업이 정본 파일을 교체하면 hard link가 분리될 수 있으므로 pull이나 checkout 뒤 `install.ps1`과 `verify.ps1`을 다시 실행한다.
